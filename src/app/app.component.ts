...
export class AppComponent implements OnInit, AfterContentInit {
  ...
  openFilter(col: string) {
    const element = document.getElementById(col + '-filter');
    if (element) {
    ...
    if (this.filterText != '') {
      if (this.filterText.trim() != '') {
        this.dataSource.data = this.data
          ...
          .filter(
            (element) =>
              JSON.stringify(element[col]).indexOf(this.filterText) > -1
          );
      }
    } else {
      this.dataSource.data = this.data;
